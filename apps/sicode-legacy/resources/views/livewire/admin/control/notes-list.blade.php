    ...
    <div class="container-fluid">
        <div class="oexterno-header d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">
            <div>
                ...
                <div class="meta">Controle Notes</div>
            </div>
        </div>
                            ...
                            <div class="row g-2">
                                ...
                                <div class="col-12 col-sm-8">
                                    <div class="form-floating w-100 position-relative">
                                        ...
                                        <label for="search">Buscar por nota, cliente, pedido ou PEP</label>
                                        <button
                                            class="btn btn-outline-secondary position-absolute end-0 top-50 translate-middle-y me-2"
                                            data-bs-toggle="modal" data-bs-target="#buscar_multi_notes">
                    ...
                    <div class="col-12 col-lg-5 col-xl-6">
                        <div class="filter-card h-100">
                            ...
                            <div class="text-muted small">
                                Busca individual por numero da nota, cliente, pedido ou PEP.
                                Para busca em lote, informe apenas numeros da nota separados por virgula, espaco,
                                ponto e virgula, tabulacao ou quebra de linha.
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="table-card">
                    ...
                    @else
                        <div class="card-header fw-bold text-bg-secondary d-flex justify-content-between align-items-center">
                            <h4 class="mb-0">LISTA DE NOTES</h4>
                        </div>
                            ...
                            <table class="table table-sm table-hover table-striped mb-0">
                                <thead class="table-dark">
                                    <tr class="sticky-top bg-dark" style="z-index:1; top:0;">
                                        ...
                                        <th class="fw-bold text-start">Cliente</th>
                                        <th class="fw-bold text-center">Pedido</th>
                                        <th class="fw-bold text-center">PEP</th>
                                        <th class="fw-bold text-center">Status</th>
                                        <th class="fw-bold text-center">Nstats</th>
                                        <th class="fw-bold text-center">Center job</th>
                                        <th class="fw-bold text-center">Data status</th>
                                ...
                                <tbody>
                                    @foreach ($lists as $list)
                                        <tr class="align-middle text-center">
                                            <td class="text-start">
                                                <div class="fw-semibold">{{ $list->note ?? '---' }}</div>
                                                <div class="small text-muted">{{ $list->nexp ?? '---' }}</div>
                                            </td>
                                            <td class="text-start">
                                                <div class="fw-semibold">{{ $list->client ?? '---' }}</div>
                                                <div class="small text-muted">{{ $list->group1 ?? '---' }}</div>
                                            </td>
                                            <td>{{ $list->numPedido }}</td>
                                            <td>{{ $list->pep }}</td>
                                            <td>{{ $list->status }}</td>
                                            <td>{{ $list->nstats }}</td>
                                            <td>{{ $list->centerjob }}</td>
                                            <td>{{ $list->dt_status?->format('d/m/Y H:i') }}</td>
                                            <td>{{ $list->value }}</td>
                                            <td>
                                                <button class="btn btn-sm btn-primary p-1"
                                                    wire:click="$emitTo('admin.control.notes-edit', 'getInfoResponse', {{ $list->id }})">
                                                    Editar
                                                </button>
                                            </td>
                                        </tr>
        ...
        <div wire:ignore.self class="modal fade" id="buscar_multi_notes" tabindex="-1"
            aria-labelledby="buscarMultiNotesLabel" aria-hidden="true">
        ...
        @livewire('admin.control.notes-edit', key('admin-notes-edit'))
    </div>
</div>
