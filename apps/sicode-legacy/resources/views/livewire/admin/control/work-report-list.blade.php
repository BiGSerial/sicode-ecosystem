    ...
    <div class="container-fluid">
        <div class="oexterno-header d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">
            <div>
                ...
                <div class="meta">Controle WorkReports</div>
            </div>
        </div>
                                ...
                                <div class="col-12 col-sm-8">
                                    <div class="form-floating w-100 position-relative">
                                        <input wire:model.debounce.500ms="search" type="text"
                                            ...
                                            placeholder="Buscar">
                                        <label for="search">Buscar por nota, ordem, dd, informer, responsável ou id</label>
                                        <button
                                            class="btn btn-outline-secondary position-absolute end-0 top-50 translate-middle-y me-2"
                                            data-bs-toggle="modal" data-bs-target="#buscar_multi_workreport">
                            ...
                            <table class="table table-sm table-hover table-striped mb-0">
                                <thead class="table-dark">
                                    <tr class="sticky-top bg-dark" style="z-index:1; top:0;">
                                        ...
                                        <th class="fw-bold text-center">Informer</th>
                                        <th class="fw-bold text-center">Informado em</th>
                                ...
                                <tbody>
                                    @foreach ($lists as $item)
                                        <tr class="align-middle text-center">
                                            <td class="text-start">
                                                <div class="fw-semibold">ID: {{ $item->id }}</div>
                                                <div class="small text-muted">Nota: {{ $item->Note?->note ?? '---' }}</div>
                                            </td>
                                            <td>{{ $item->Company?->name ?? '---' }}</td>
                                            <td>{{ $item->User?->name ?? '---' }}</td>
                                            <td>{{ $item->dd ?? '---' }}</td>
                                            <td>{{ $item->informer ?? '---' }}</td>
                                            <td>{{ $item->informed_at?->format('d/m/Y H:i') ?? '---' }}</td>
                                            <td>
                                                @if ($item->approved)
                                            ...
                                            <td class="text-end">
                                                <button class="btn btn-sm btn-primary"
                                                    wire:click="$emitTo('admin.control.work-report-edit', 'getInfoResponse', {{ $item->id }})">
                                                    Editar
                                                </button>
        ...
        <div wire:ignore.self class="modal fade" id="buscar_multi_workreport" tabindex="-1"
            aria-labelledby="buscarMultiWorkreportLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content edp-bg-stategrey-50">
                    <div class="modal-header edp-bg-sprucegreen-70 text-edp-verde">
                        Buscar Multi-WorkReports
                    </div>
                    <div>
        ...
        @livewire('admin.control.work-report-edit', key('admin-workreport-edit'))
    </div>
</div>
