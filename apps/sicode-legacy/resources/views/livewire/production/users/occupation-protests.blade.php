<div wire:poll>
    ...
    @php
        $total = max(1, $lists->sum('total_user') + $lists->sum('total_monitoring'));
            ...
            <div class="card-body p-2">
                ...
                <div class="mb-2" wire:ignore.self>
                    <div class="input-group input-group-sm">
                        <span class="input-group-text border-secondary bg-light"><i class="ri-search-line"></i></span>
                        <input type="text" class="form-control form-control-sm border-secondary"
                            placeholder="Filtrar por usuário" wire:model.debounce.300ms="search" aria-label="Filtrar">
                    </div>
                </div>
                ...
                <ul class="list-group list-group-flush slim-scroll">
                    @foreach ($lists as $indice => $list)
                        {{-- @dd($list) --}}
                        @php
                            $parts = explode(' ', trim($list->User?->name));
                            $first = $parts[0] ?? '';
                            $last = count($parts) > 1 ? end($parts) : '';
                            $display = trim($first . ' ' . $last);
                            $initials = mb_strtoupper(mb_substr($first, 0, 1) . mb_substr($last, 0, 1));
                            $perc = $pct($list->total_monitoring + $list->total_user);
                            $rank = $indice + 1;
                        ...
                        <li class="list-group-item py-1 px-2 sidebar-row" role="button"
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div class="position-relative flex-shrink-0">
                                    ...
                                    {{-- Watchdog dot --}}
                                    @if (isset($list->User->Watchdog) && $list->User->Watchdog->watchdog)
                                ...
                                <div class="flex-grow-1 min-w-0">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        <small
                                            class="text-muted">{{ $list->total_monitoring + $list->total_user }}</small>
                                        <small class="text-muted"><i class="ri-eye-line align-middle"></i>:
                                            {{ $list->total_monitoring }} • <i class="ri-user-line align-middle"></i>:
                                            {{ $list->total_user }}</small>
                                    </div>
                                </div>
                            </div>
                        </li>
