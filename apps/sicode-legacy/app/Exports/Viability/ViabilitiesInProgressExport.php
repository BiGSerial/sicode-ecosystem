...
class ViabilitiesInProgressExport implements FromQuery, WithMapping, WithHeadings, WithProperties, WithEvents, WithChunkReading, ShouldQueue
{
    use Exportable;
    ...
    public function map($viab): array
    {
        ...
        return [
            ...
            $viab->sended_at?->format('d/m/Y'),
            $viab->sended_at?->format('H:i'),
            $viab->sended_at?->addDays($viab->getDays() + 7)->format('d/m/Y'),
            $viab->getDays(),
