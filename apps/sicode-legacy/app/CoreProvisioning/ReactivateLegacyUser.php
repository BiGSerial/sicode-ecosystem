...
final class ReactivateLegacyUser
{
    ...
    public function __invoke(string $coreSubject, array $payload, string $clientIdentifier, string $applicationContext): ProvisioningOutcome
    {
        ...
        $idempotencyKey = (string) ($payload['idempotency_key'] ?? 'idem-reactivate-user');

        $lockKey = implode(':', [
            'core-provisioning-user-reactivate',
        ...
        return $this->lock->withLock($lockKey, function () use ($coreIssuer, $coreSubject, $applicationContext): ProvisioningOutcome {
            return DB::transaction(function () use ($coreIssuer, $coreSubject, $applicationContext): ProvisioningOutcome {
                ...
                if ($link->status === CoreIdentityLink::STATUS_ACTIVE) {
                    return new ProvisioningOutcome(ProvisioningOutcome::RESULT_ALREADY_ACTIVE, 'user', [
                        'core_subject' => $coreSubject,
                        'user_id' => $link->legacy_user_id,
                    ]);
                }

                $link->forceFill(['status' => CoreIdentityLink::STATUS_ACTIVE])->save();

                return new ProvisioningOutcome(ProvisioningOutcome::RESULT_REACTIVATED, 'user', [
                    'core_subject' => $coreSubject,
                    'user_id' => $link->legacy_user_id,
                ]);
            });
